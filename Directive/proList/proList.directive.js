    ...
    return {
        ...
        controller:function($http,$scope,enume,$state){
            ...
            $scope.bzzy = function (item) {
                ...
                $state.go("roomManage.homeworkCreate", { entity: { tag: "add", homeworkType: 0, KCBH: item.code, Type: 0} });

            }

            $scope.selectAll = function(){
                for(var i =0;i<$scope.proList.length;i++){
                    $scope.proList[i].ck = $scope.ckAll;
                }
            }
            ...
            $scope.selectOne = function(){
                var len = $scope.proList.length;
                var slen = getSelectedItems($scope.proList).length;
